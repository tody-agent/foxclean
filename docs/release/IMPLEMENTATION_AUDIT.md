...
| GUI shell | SwiftUI sidebar sections, dashboard, onboarding with `FOX_SKIP_ONBOARDING` dev bypass, settings with appearance/accent/privacy controls, toolkit views, mascot component, and AppKit menu bar status item. |
...
| Accessibility pass | Static labels exist for the sidebar, Full Disk Access footer, treemap, dashboard storage/scan gauges, optimizer result rows, menu bar chart/buttons, and status item. Foxie and dashboard gauges honor macOS Reduce Motion, with an app-level "Reduce Foxie animations" toggle. Full VoiceOver audit remains a release task. |
...
| Clean UI static guard | `script/check_clean_ui_static.sh` verifies cleanup categories, sidebar category rendering, search/select/deselect controls, destructive confirmations, app filter toggles, uninstall sensitivity, install metadata, protected app hiding, orphan Smart Select, orphan safety policy, and Finder reveal affordances. |
...
| Toolkit UI static guard | `script/check_toolkit_ui_static.sh` verifies Project Purge grouping, Recent badge/default-off behavior, destructive confirmations, Installer source filters, size/age sorting, old-installer auto-selection, and whitelist editor access. |
