...
| GUI onboarding/dashboard/settings | SwiftUI views under `FoxCleanApp/Views`; `script/check_gui_static.sh` verifies onboarding page flow, `FOX_SKIP_ONBOARDING` dev bypass, Full Disk Access controls, dashboard scan/clean states, destructive clean confirmation, category toggles, settings tabs/toggles for appearance, accent color, privacy/FDA, startup, menu bar, scanning, safety, accessibility, scheduling, repository links, and PureMac/Mole credits. App build and launch verification pass. | Pass |
...
| Clean/uninstall/orphans UI | PureMac-derived app views and services exist; `script/check_clean_ui_static.sh` verifies at least 9 cleanup categories, sidebar category rendering, search/select/deselect controls, destructive confirmations, app search/filter toggles, uninstall sensitivity slider, install-date/protection metadata, orphan search and Smart Select, orphan safety policy, remove actions, and Finder reveal affordances. | Pass |
