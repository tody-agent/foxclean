...
/// Detects whether Full Disk Access (FDA) has been granted to FoxClean.
...
final class FullDiskAccessManager {
    ...
    /// the OS records FoxClean as the requester and adds it to the FDA pane.
    ...
    /// Force FoxClean to appear in the Full Disk Access list.
    ///
    ...
    /// *Finder* — not FoxClean. So at launch we touch a few protected paths
    ...
    /// Reveal the running FoxClean.app bundle in Finder so the user can drag
    ...
    /// Reset FoxClean's TCC entries so the OS can re-register the bundle.
    ...
    func resetFullDiskAccess() -> Bool {
        let bundleID = Bundle.main.bundleIdentifier ?? "com.foxclean.app"
        let process = Process()
