...
extension Notification.Name {
    static let foxCleanRoute = Notification.Name("FoxCleanRoute")
    static let foxCleanSmartScan = Notification.Name("FoxCleanSmartScan")
}
...
final class AppState: ObservableObject {
    ...
    init() {
        ...
        if UserDefaults.standard.bool(forKey: "FoxClean.OnboardingComplete") {
            scheduler.start()
        }
        notificationTokens.append(
            NotificationCenter.default.addObserver(
                forName: .foxCleanSmartScan,
                object: nil,
                queue: .main
            ) { [weak self] _ in
                Task { @MainActor in
    ...
    func removeSelectedFiles() {
        ...
        trashDirectly(urls: urls) { [weak self] removed, failed in
            DispatchQueue.main.async {
                ...
                if !failed.isEmpty {
                    self.removalError = "\(failed.count) file\(failed.count == 1 ? "" : "s") could not be removed. Grant Full Disk Access in System Settings → Privacy & Security to allow FoxClean to manage all files."
    ...
    /// originates from FoxClean itself — TCC then registers FoxClean in the
    ...
    /// FDA to FoxClean made no difference (issue #75).
    ...
    func route(to url: URL) {
        guard url.scheme == "foxclean" else { return }
        let token = url.host?.isEmpty == false ? url.host! : url.pathComponents.dropFirst().first ?? "dashboard"
        route(to: token)
    }

    func route(to token: String) {
        ...
        switch normalized {
        case "dashboard", "smart-scan", "smartscan":
            section = .cleaning(.smartScan)
        case "apps", "applications", "installed-apps":
            section = .apps
        case "orphans", "orphaned-files":
            section = .orphans
        case "analyzer", "disk", "disk-analyzer":
            section = .analyzer
        case "monitor", "status", "system-monitor":
            section = .monitor
        case "installers", "installer":
            section = .installers
        case "projects", "purge", "project-purge":
            section = .projects
        case "optimize", "optimizer":
            section = .optimize
        default:
            if let category = CleaningCategory.allCases.first(where: { category in
                category.rawValue
                    .lowercased()
                    .replacingOccurrences(of: "&", with: "")
        ...
        if let section {
            NotificationCenter.default.post(name: .foxCleanRoute, object: section)
        }
    }
